...
async function shot(filename, label, setupFn) {
  ...
  const info = await page.evaluate(() => {
    ...
    return {
      phaseLabel: lbl ? lbl.textContent : '',
      countText: count ? count.textContent : '',
      orbTransform: img ? img.style.transform : '',
      orbTransition: img ? img.style.transition : '',
    };
  });
...
(async () => {
  // v9-a: mid-inhale, count "3", orb scale mid-way (voice + visual sync state)
  await shot('s2-v9-voice-driven.png', 'inhale-count3-mid', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    if (img) {
      img.style.transition = 'transform 600ms linear';
      img.style.transform = 'scale(0.9888)';  /* 0.92 + (1.05-0.92)*0.53 — mid-inhale */
    }
    ...
    seg0.querySelector('.s2-timeline-fill').style.width = '75%';
    seg0.querySelector('.s2-timeline-count').textContent = '3';
  });

  // v9-b: exhale, count "2", completed In+Hold
  await shot('s2-v9-exhale-count2.png', 'exhale-count2', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    if (img) {
      img.style.transition = 'transform 600ms linear';
