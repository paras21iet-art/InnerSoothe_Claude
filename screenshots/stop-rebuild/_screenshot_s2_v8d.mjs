...
async function shot(filename, label, setupFn) {
  ...
  console.log(`Saved: ${filename}  [${label}]  timeline: top=${geo?.top} height=${geo?.height} width=${geo?.width}`);
  await browser.close();
}

(async () => {
  // V1-a: cycle 1 inhale active (segment 0)
  await shot('s2-v8d-tl-inhale.png', 'inhale-active', () => {
  ...
  // V1-b: cycle 1 hold-top active (segment 1)
  await shot('s2-v8d-tl-hold-top.png', 'hold-top-active', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    ...
    seg1.classList.add('active'); seg1.querySelector('.s2-timeline-fill').style.width = '50%';
  });

  // V1-c: cycle 1 exhale active (segment 2)
  await shot('s2-v8d-tl-exhale.png', 'exhale-active', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    ...
    seg2.classList.add('active'); seg2.querySelector('.s2-timeline-fill').style.width = '50%';
  });

  // V1-d: cycle 1 hold-bottom active (segment 3)
  await shot('s2-v8d-tl-hold-bot.png', 'hold-bottom-active', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    ...
    seg3.classList.add('active'); seg3.querySelector('.s2-timeline-fill').style.width = '50%';
  });
