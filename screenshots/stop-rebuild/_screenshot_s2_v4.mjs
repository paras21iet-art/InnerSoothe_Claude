...
(async () => {
  // s2-15: mid-phase, bold countdown number "3", inhale phase
  await shot('s2-15-counting-mid-phase.png', () => {
  ...
  // s2-16: pattern pill bold — full screen, orb hold-bottom, showing pill
  await shot('s2-16-pattern-pill-bold.png', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
  ...
  // s2-17: entry intro — orb at scale 0.92 (waiting class), CTA disabled, "One more cycle" hidden
  await shot('s2-17-orb-neutral.png', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    ...
    const cyc = document.querySelector('.sec-stop-s2 .s2-cycle');
    if (cyc) cyc.textContent = 'Cycle 1 of 4';
    // CTA disabled, "One more cycle" hidden — both already in default HTML state
  });
