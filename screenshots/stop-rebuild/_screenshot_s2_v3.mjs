...
(async () => {
  // s2-13: during utterance 1 — orb neutral (waiting), CTA disabled, "One more cycle" hidden
  // Phase label shows "Breathe in", cycle "Cycle 1 of 4" — intro state
  await shot('s2-13-intro-utterance1.png', () => {
    ...
    // CTA disabled (set in HTML), "One more cycle" hidden (no .visible class)
  });

  // s2-14: during utterance 2 — same visual state (orb still neutral, about to begin)
  await shot('s2-14-intro-utterance2.png', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    if (img) img.className = 's2-hero-img waiting';
    const lbl = document.querySelector('.sec-stop-s2 .s2-phase-label');
    if (lbl) lbl.textContent = 'Breathe in';
