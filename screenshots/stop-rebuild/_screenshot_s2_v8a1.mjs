...
(async () => {
  // s2-25: mid-inhale, count "2" — voice-visual locked via utterance.onstart (v8a.1)
  await shot('s2-25-voice-visual-locked.png', () => {
    const img = document.querySelector('.sec-stop-s2 .s2-hero-img');
    if (img) {
      img.className = 's2-hero-img inhale';
      img.style.transform = 'scale(0.985)';  // linear midpoint: 0.92 + 0.130*0.5
    }
